...
void SimpleLeptonOp::do_execute() 
{
  ...
  for (auto& mu : event.muons) {
    ...
    bool isMvaMedium     = mu.mvaMedium;
    bool isMvaTight      = mu.mvaTight;
    bool isMiniIsoMedium = mu.miniIsoMedium;
    bool isMiniIsoTight  = mu.miniIsoTight;
  ...
  for (auto& ele : event.electrons) {
    ...
    if (analysis.hbb) {
      ...
      if (pt<7 || aeta>2.4 || fabs(ele.dxy)>0.05 || fabs(ele.dz)>0.2)
        continue;
      miniRelIso = MiniRelIso(ele, pfCandsMap.get());
  ...
  for (auto& mu : event.muons) {
    ...
    if (analysis.hbb) {
      if (pt<5 || aeta>2.4 || !mu.loose || fabs(mu.dxy)>0.5 || fabs(mu.dz)>1.0) 
        continue;
      miniRelIso = MiniRelIso(mu, pfCandsMap.get(), event.rho);

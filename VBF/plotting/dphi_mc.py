...
parser.add_argument('--proc',metavar='proc',type=str,default=None)
...
weight = sel.weights[args.region]%1
...
### DEFINE PROCESSES ###
mc_in       = Process("0.2 < #Delta#phi < 0.5",root.kData)
mc_in.additional_cut = 'fabs(jot12DPhi)>0.2 && fabs(jot12DPhi)<0.5'
mc_in.additional_weight = weight
mc_lo       = Process("#Delta#phi < 0.2",root.kData,3)
mc_lo.additional_cut = 'fabs(jot12DPhi)<0.2'
mc_lo.additional_weight = weight
mc_lo.ratio = True
mc_out       = Process("0.5 < #Delta#phi < 0.8",root.kData,2)
mc_out.additional_cut = 'fabs(jot12DPhi)>0.5 && fabs(jot12DPhi)<0.8'
mc_out.additional_weight = weight
mc_out.ratio = True

### ASSIGN FILES TO PROCESSES ###
for d in [mc_in, mc_out, mc_lo]:
    d.add_file(baseDir+'/'+args.proc+'.root')
    plot.add_process(d)

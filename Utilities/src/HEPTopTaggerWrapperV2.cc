...
PseudoJet HEPTopTaggerV2::result(const PseudoJet & jet) const{
  ...
  htt::HEPTopTaggerV2 tagger(jet);
  
  htt::HEPTopTaggerV2 best_tagger;
  ...
  tagger.set_mode((htt::Mode)mode_);
    ...
    for (int iq = 0; iq < niter; iq++) {
      ...
      if (tagger.is_tagged()) {
        ...
        if (tagger.q_weight() > weight_q1) // {
          best_tagger = tagger;
        weight_q1=tagger.q_weight();  // don't ask
//         }
      }
    }
